...
pub async fn import_timezone(db: &Db, file_path: &str) -> Result<()> {
    ...
    if imported_count == 0 {
        ...
        println!("Successfully imported {} timezone(s)", imported_count);
    }
...
pub async fn set_calendar_timezone(db: &Db, calendar_id: i64, tzid: &str) -> Result<()> {
    ...
    match timezone {
        Some(tz) => {
            ...
            db.timezones().associate_with_calendar(calendar_id, tz.id).await?;
            println!("Set timezone '{}' for calendar '{}'", tz.tzid, calendar.name);
            Ok(())
        }
        None => {
            println!("Timezone '{}' not found", tzid);
