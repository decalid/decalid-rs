...
impl Config {
    pub fn with_port(self, port: u16) -> Self {
        Self { server: ServerConfig { port, ..self.server }, ..self }
    }

    pub fn with_host(self, host: &str) -> Self {
        Self { server: ServerConfig { host: host.to_string(), ..self.server }, ..self }
    }
}
...
impl Default for WebauthnConfig {
    fn default() -> Self {
        Self {
            ...
            rp_origin: reqwest::Url::parse("https://cal.decalid.com").expect("Failed to parse RP origin"),
